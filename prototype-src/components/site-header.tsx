...
import { MobileNav } from "@prototype/components/mobile-nav";
import { NavLinks } from "@prototype/components/nav-links";
import { UserMenu } from "@prototype/components/user-menu";
import { SignOutMenuItem } from "@prototype/components/sign-out-button";
import { getUnreadNotificationCount } from "@prototype/lib/notifications";
      ...
      <Container className="flex h-[72px] items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden items-center gap-2 border-r border-border pr-2 mr-1 lg:flex">
            <Link href="/prototype/business/dashboard" className="px-2 text-xs font-semibold text-muted-foreground hover:text-foreground">
              Business
            </Link>
            <Link href="/prototype/admin/dashboard" className="px-2 text-xs font-semibold text-muted-foreground hover:text-foreground">
              Admin
            </Link>
          </div>
          ...
          {session?.user ? (
            ...
            <div className="hidden items-center gap-2 sm:flex">
              <Button asChild variant="ghost">
                <Link href="/prototype/auth/sign-in">Sign in</Link>
              </Button>
              <Button asChild>
                <Link href="/prototype/auth/create-account">Create account</Link>
              </Button>
            </div>
          )}
